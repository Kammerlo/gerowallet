// DEV ONLY: VITE_FLUXPOINT_API_KEY exposes the key in the client bundle; production must use the Nexus proxy and must NOT set it.
...
// DEV ONLY: direct-to-Fluxpoint fallback. Active only when VITE_FLUXPOINT_API_KEY is set.
// Uses a SEPARATE axios instance so the api-key header is never sent to Nexus.
const FLUXPOINT_API_KEY: string = import.meta.env['VITE_FLUXPOINT_API_KEY'] || '';
