<template>
  <v-dialog content-class="rounded-xxl dialogStyle" v-model="dialogLocal" scrollable max-width="850" :persistent="loading">
    <v-card class="py-0 rounded-xxl transparent fill-height">
      <v-card-title style="word-break: break-word">Swap
      <v-spacer></v-spacer>
      ...
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn color="primary" text :disabled="loading" @click="dialogLocal = false">
          Cancel
        </v-btn>
        <v-btn :disabled="loading" color="primary" elevation="0" :loading="loading" @click="addTokens">
          Add
        </v-btn>
        <v-spacer></v-spacer>
      </v-card-actions>
      ...
      >
        <v-card v-if="loading && !txError" flat class="transparent text-center justify-center">
          ...
          <v-card-subtitle v-if="txState === 'Done!'" class="pt-0 text-center" style="word-break: break-word; color: white; font-size: 12px; line-height: 2">
          ...
          <v-btn v-if="txState === 'Done!'" text plain :href="network === 'preview' ? 'https://preview.cexplorer.io/tx/'+txId : 'https://cexplorer.io/tx/'+txId" target="_blank">
            {{ txId | truncate }}
          </v-btn>
          <v-progress-linear
              v-if="txState === 'Awaiting Signature ...' || txState === 'Minting ...' || txState === 'Verifying Transaction ...'"
              buffer-value="0"
              color="primary"
              stream
              value="0"
          ...
          <v-card-actions class="text-center justify-center">
            <v-btn
                v-if="txState === 'Done!'"
                color="green"
                @click="reload(); dialogLocal = false"
            >
              Close
            </v-btn>
          </v-card-actions>
    ...
    async addTokens() {
      ...
      try {
        ...
        if (error.info) {
          ...
          this.$emit('snackbarChange', { text: 'Transaction Signing Error Please Contact Adabox Team.', color: 'error' })
        }
        this.loading = false
        return
      }
