...
export default {
  ...
  async createNewWallet(name, icon, theme, mnemonic: string, password, chain, network) {
    ...
    const words = bech32.toWords(Buffer.from(xpubHex, 'hex')) ?? bech32m.toWords(Buffer.from(xpubHex, 'hex'));
  ...
  async createNewWalletDb(walletId: number, hasEncryptedMnemonic: boolean, isRestore: boolean = false) {
