...
db.version(10).stores({
  wallets:
    "++walletId, name, icon, type, theme, order, encryptedPrivateKey, publicKey, passwordLastUpdate, chain, network",
});
...
export default {
  ...
  async saveAccountDefaultDerivations(chainNetworkId, publicKey, walletId) {
    ...
    const stakingKey = publicKey.derive(ChainDerivations.CHIMERIC_ACCOUNT).derive(STAKING_KEY_INDEX).to_raw_key();
