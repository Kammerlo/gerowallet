...
// Byte-for-byte conformance vector shared with the iOS approver. iOS MUST
// reproduce COSE_SIGN1 / COSE_KEY exactly (Ed25519 is deterministic; the COSE
// CBOR must match). Inputs: wallet STAKE key = 32 bytes of 0x01 (mainnet),
// relay-auth pubkey = RELAY_PUB, deviceId = DEVICE_ID (= 4884fdaafea47c29fea7159d0daddd9c).
const VEC_STAKE = 'stake1uyxk54m7j3q6mrkevcunryrwf4p7e68c93cjk8gzxkhlkpswtcyrc';
const VEC_COSE_SIGN1 = '';
...
describe('DEVICE_REGISTER proof conformance vector (byte-for-byte with iOS)', () => {
  it('reproduces the exact reward address + COSE bytes', () => {
    const wallet = walletStakeKey(1);
    const stake = rewardAddressOf(wallet);
    expect(stake).toBe(VEC_STAKE);
    const subject = buildDeviceRegisterSubject(DEVICE_ID, RELAY_PUB, stake);
    const proof = makeProof(wallet, stake, subject);
    expect(proof.coseSign1).toBe(VEC_COSE_SIGN1);
    expect(proof.coseKey).toBe(VEC_COSE_KEY);
  });

  it('the pinned conformance proof verifies', async () => {
