<template>
        ...
        <v-row no-gutters>
          ...
          <v-col cols="6" class="collectibles px-2" v-if="collections?.length > 0">
            <v-card flat outlined>
              <v-card-title class="justify-center">{{ $t('multisig.chooseCollectibles') }}</v-card-title>
              <v-card-subtitle class="pb-0">
                <v-text-field
                  v-model="search"
                  :placeholder="$t('multisig.searchForCollectibles')"
                  outlined
                  dense
                  hide-details
                  class="mb-4"
  ...
  const tokenModel = computed({
    get: () => props.value?.selectedTokens || [],
    set: (newTokens: Token[]) => {
      emit('input', {
        ...props.value,
  ...
  const getAvailableTokens = (currentIndex: number) => {
  ...
  const decreaseQuantityToSend = (item: Collectible) => {
    if (item.toSendQuantity > 1) {
      item.toSendQuantity--;
    }
  };

  const increaseQuantityToSend = (item: Collectible) => {
    if (item.toSendQuantity < item.quantity) {
      item.toSendQuantity++;
    }
  };

  const removeTokenSelector = (index: number) => {
