<template>
  ...
  export default defineComponent({
      ...
      async signAndSubmitTx() {
        const signAndReturnTx = async () => {
          ...
          try {
            ...
            console.log(signedTx.to_json())
            const txId = await appWallet.submitTx(signedTx, this.utxos, this.isMultisig);
            console.log(txId)
            snackbar.fireSuccess(`Tx Submitted Successfully. Tx ID: ${txId}`)
            this.$emit('close')
          } catch (e) {
    ...
    mounted() {
      console.log("this is the recipientAddress mounted object::::", this.recipientAddressProp);
      if(this.isMultisig) {
        this.sendData = { ...this.sendData, recipientAddress: this.recipientAddressProp};
        this.nextStep();
      }
      if (this.resolvedAssets) {
        // this.sendData.recipientAddress = this.recipientAddressProp;
        // this.sendData = { ...this.sendData, recipientAddress: this.recipientAddressProp};
    ...
    created() {
      /*console.log("this is the recipientAddresss created object::::", this.recipientAddressProp);
      this.sendData = { ...this.sendData, recipientAddress: this.recipientAddressProp};
      if(this.isMultisig) {
        console.log("created is multisig and currenttesp1");
        this.sendData = { ...this.sendData, recipientAddress: this.recipientAddressProp};
        this.nextStep();
      }*/
    },
