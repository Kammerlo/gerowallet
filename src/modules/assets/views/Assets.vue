  ...
  <v-layout>
    <v-row no-gutters>
      ...
      <v-col cols="12" class="pa-2">
        <TokenAllocationTable :assets="computedAssets" :collectibles="computedCollectibles" @click="handleOnRowClick"></TokenAllocationTable>
      </v-col>
    </v-row>
...
export default {
  ...
  computed: {
    ...mapState(useStore, ["utxos", "assets"]),
    computedTokens() {
      return this.tokens;
    },
    computedAssets() {
      return this.computedTokens.filter(asset => asset?.metadata || asset?.name === "ADA");
    },
    computedCollectibles() {
      return this.computedTokens.filter(asset => !asset?.metadata && asset?.name !== "ADA");
    },
    computePieChartData() {
      const totalTokens = this.computedTokens.length || 1; // Avoid division by zero
      return [
        ["Assets", (this.computedAssets.length / totalTokens) * 100],
        ["Collectibles", (this.computedCollectibles.length / totalTokens) * 100],
      ];
    },
  },
  async mounted() {
    ...
    this.tokens = await this.aggregateAssets();
  },
  methods: {
