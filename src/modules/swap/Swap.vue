        ...
        <v-col cols="12" xl="8" lg="8">
          <v-card flat outlined>
            ...
            <v-card-text class="text-center justify-center">
              <v-card flat max-width="400" outlined style="margin: auto">
                <v-card-title>
                  <v-btn-toggle mandatory active-class="highlight" v-model="swapType">
                    <v-btn small color="black" :value="0" rounded style="text-transform: capitalize">
                      Swap
                    </v-btn>
                    <v-btn small color="black" :value="1" rounded style="text-transform: capitalize" :disabled="true" >
                      Limit
                    </v-btn>
                    <v-btn small color="black" :value="1" rounded style="text-transform: capitalize" :disabled="true" >
                      DCA
                  ...
                  <v-btn-toggle v-model="settingsToggle">
                    <v-btn small rounded>
                      ...
                      <span v-else>{{ slippage === 'auto' ? 'AUTO' : slippage+'%'}}</span>
                      &nbsp;<v-icon small>mdi-cog</v-icon>
                    </v-btn>
                  </v-btn-toggle>
                </v-card-title>
                <v-card-text>
                  <TokenSelector v-model="selectedTokens[0]" :available="tokens" :index="0" title="Selling" titleColor="#FDA29B"></TokenSelector>
                  <v-btn outlined icon color="#00DFF3" style="margin-top: 8px; z-index: 5">
                    <v-icon color="#00DFF3">
                      mdi-chevron-double-down
                    </v-icon>
                  </v-btn>
                  <TokenSelector style="margin-top: -18px" v-model="selectedTokens[0]" :available="tokens" :index="0" title="Buying" titleColor="#75E0A7" background-color="transparent" :max-button-enabled="false"></TokenSelector>
                  <div class="text-left">
                    <v-btn text plain style="letter-spacing: normal;" :ripple="false" class="px-0 no-opacity">
                      1 GERO = 0.003 ADA
                    </v-btn>
                  </div>
                  <v-btn color="primary" large block rounded style="border-radius: 10px">Swap</v-btn>
                </v-card-text>
                <v-overlay
                  absolute
                  :value="settingsToggle == 0"
                  opacity="0.94"
                ...
                >
                  <v-card class="fill-height transparent">
                    <v-card-title>
                      <v-btn small icon @click="settingsToggle = undefined">
                    ...
                    <v-card-text class="d-flex justify-space-around justify-center flex-column" style="height: calc(100% - 64px)">
                      <div class="d-flex justify-space-between justify-center">
                        <span class="d-flex flex-column">
                          ...
                          <v-btn class="px-0 justify-start" text plain :ripple="false" small style="text-transform: capitalize; word-break: break-word; letter-spacing: normal" href="https://www.investopedia.com/terms/s/slippage.asp" target="_blank">
                            Learn more
                          </v-btn>
                        </span>
                        <v-btn-toggle mandatory active-class="geroButton" class="transparent" v-model="slippageToleranceType" style="align-items: center;">
                      ...
                      <div class="d-flex justify-center">
                        <v-btn-toggle mandatory active-class="geroButton" class="transparent" v-model="slippageTolerance" style="align-items: center;" @change="onSlippageToleranceChange">
                          <v-btn rounded :value="0.5" class="pa-2" style="border-color: black!important;" height="48" :disabled="automaticSlippage == 'on' || unlimitedSlippage == 'on'">
                            0.5
                          </v-btn>
                          <v-btn rounded :value="1" class="pa-2" style="border-color: black!important;" height="48" :disabled="automaticSlippage == 'on' || unlimitedSlippage == 'on'">
                            1
                          </v-btn>
                          <v-btn rounded :value="2" class="pa-2" style="border-color: black!important;" height="48" :disabled="automaticSlippage == 'on' || unlimitedSlippage == 'on'">
                            2
                          </v-btn>
                          <v-btn rounded :value="5" class="pa-2" style="border-color: black!important; height: 48px" :disabled="automaticSlippage == 'on' || unlimitedSlippage == 'on'">
                            5
                          </v-btn>
                          <v-btn rounded :value="-1" class="py-2 px-0" style="border-color: black!important; height: 48px" :disabled="automaticSlippage == 'on' || slippageToleranceType == 'auto' || unlimitedSlippage == 'on'">
                            <v-text-field
                              ...
                              <template v-slot:append>
                                <span :style="slippageTolerance === -1 ? {color: 'black'} : automaticSlippage == 'on' || slippageToleranceType == 'auto' || unlimitedSlippage == 'on' ? {color: 'white', opacity: '0.3'} : {color: 'white'}">%</span>
                              </template>
                            </v-text-field>
                          </v-btn>
                        </v-btn-toggle>
                      ...
                      <div class="d-flex justify-space-between justify-center">
                        <span class="d-flex flex-column">
                          ...
                          >
                            <template v-slot:activator="{ }">
                              <v-btn class="px-0 justify-start" text plain :ripple="false" small style="text-transform: capitalize; word-break: break-word; letter-spacing: normal" @click="unlimitedSlippageTooltipEnabled = true" v-click-outside="disableUnlimitedSlippageTooltip">
                                Learn more
                              </v-btn>
                            </template>
                        ...
                        <v-btn-toggle mandatory active-class="geroButton" class="transparent" v-model="unlimitedSlippage" style="align-items: center;" @change="unlimitedSlippageChange">
                          <v-btn small color="black" :value="'off'" rounded style="text-transform: capitalize" class="pa-2">
                            OFF
                          </v-btn>
                          <v-btn small color="black" :value="'on'" rounded style="text-transform: capitalize" class="pa-2" :disabled="automaticSlippage === 'on'">
                            ON
                          </v-btn>
                        </v-btn-toggle>
                      </div>
                      <div class="d-flex justify-space-between justify-center">
                        <span class="d-flex flex-column">
                          ...
                          >
                            <template v-slot:activator="{ }">
                              <v-btn class="px-0 justify-start" text plain :ripple="false" small style="text-transform: capitalize; word-break: break-word; letter-spacing: normal" @click="automaticSlippageTooltipEnabled = true" v-click-outside="disableAutomaticSlippageTooltip">
                                Learn more
                              </v-btn>
                            </template>
                            <div style="width: 253px; word-break: break-word">
                        ...
                        <v-btn-toggle mandatory active-class="geroButton" class="transparent" v-model="automaticSlippage" style="align-items: center;">
                          <v-btn small color="black" :value="'off'" rounded style="text-transform: capitalize" class="pa-2">
                            OFF
                          </v-btn>
                          <v-btn small color="black" :value="'on'" rounded style="text-transform: capitalize" class="pa-2" :disabled="unlimitedSlippage === 'on'">
                            ON
                          </v-btn>
                        </v-btn-toggle>
                      </div>
...
export default defineComponent({
  ...
  computed: {
    ...
    ...mapState(useStore, ['resolvedAssets']),
    tokens() {
...
<style>
.settingsOverlay .v-overlay__content {
  width: 100%!important;
