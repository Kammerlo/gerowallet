...
export default {
  ...
  // ============================================================================
  // Physical Card Payment Methods (Placeholders for future backend integration)
  // ============================================================================

  /**
   * Get payment address for physical card shipping fee
   * Placeholder - will be implemented when backend is ready
   * @param _cardType - Type of card ('virtual' | 'physical') - reserved for future use
   * @param shippingMethod - Shipping method ('regular' | 'express-eu' | 'express-worldwide')
   * @returns Payment address and amounts
   */
  async getPaymentAddress(
    _cardType: string,
    shippingMethod: string
  ): Promise<{ address: string; amount_ada: number; amount_eur: number }> {
    // Mock data for now - will call backend API when ready
    const shippingFees: { [key: string]: { ada: number; eur: number } } = {
      regular: { ada: 12.5, eur: 3.99 },
      'express-eu': { ada: 31.2, eur: 9.99 },
      'express-worldwide': { ada: 62.5, eur: 19.99 },
    };

    const fees = shippingFees[shippingMethod] || shippingFees['regular'];

    // Placeholder address - will be generated by backend
    return {
