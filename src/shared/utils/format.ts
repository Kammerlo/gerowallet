/**
 * Canonical number/price formatting. This is the ONE home for these functions.
 *
 * scripts/design/audit.mjs counts definitions of formatPrice / formatBalance /
 * formatUsd outside this file as `formatFnForks` and ratchets that number down,
 * so new forks fail the build. Re-point a fork here rather than copying it.
 */

/** Format a number in compact notation (e.g. 1.20B, 3.50M, 800.0K) — 2dp for B/M, 1dp for K */
 ...
 * digits — e.g. 0.000006735 → "0.0₅6735".
 */
...
/**
 * Format a USD amount with thousands separators and exactly 2 decimals.
 * Pass `symbol: false` for a bare number (table cells that carry their own $ header).
 */
export function formatUsd(value: number | string | null | undefined, opts: { symbol?: boolean } = {}): string {
...
/**
 * One delta grammar: the glyph carries direction, so the color is redundant
 * reinforcement rather than the sole signal (colorblind-safe).
 * e.g. "▲ 2.4%" / "▼ 1.1%" / "0.0%"
 */
export function formatSignedChange(change: number): string {
  if (!Number.isFinite(change) || change === 0) return '0.0%';
  return `${change > 0 ? '▲' : '▼'} ${formatChange(change)}`;
}

/** Return a color string for positive/negative/zero change. Canon token values. */
export function changeColor(change: number): string {
  if (change === 0) return '#7A8088'; // --g-text-3 (was #A3A3A3)
