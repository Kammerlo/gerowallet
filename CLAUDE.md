...
- **Config**: `VITE_CIP113_PROGRAMMABLE_LOGIC_BASE_{MAINNET,PREPROD,PREVIEW}` — comma-separated `programmable_logic_base` script hashes (a list, because re-bootstrapping changes the hash while old holdings stay put). Empty = unsupported on that network, fails closed; mainnet ships empty. Read once at module scope in `src/utils/networks.ts`.
...
- **Partition**: `classifyUtxoAddress()` (`src/chrome/serialization.ts`) → `spendable` | `programmable` | `programmable-other` | `foreign`. Spendable checks run first and are unchanged; programmable UTxOs never enter coin selection or the balance total.
...
- **Display**: `walletStore.programmableTokens` → `useHoldingsValuation()` (price and value forced to 0, `isProgrammable`, distinct `rowKey` so a dual-held unit doesn't collide), rendered with a lock badge and the `programmableTokens.badge` chip.
