...
@singleton()
@autoInjectable()
export class HardwareOptionsHandler extends AbstractMessageHandler {
    ...
    async handle(request: MessageRequestInterface): Promise<void> {
        ...
        if (!maxWalletAmountAchieved) {
            if (!request.tabFound) {
                ...
                if (!tabId) {
                    chrome.tabs.create({
                        url: 'index.html?#/hardware-options'
                    });
                }
            } else {
                chrome.tabs.update(request.tabToUpdate, { selected: true, url: 'index.html?#/hardware-options' });
            }
        } else {
