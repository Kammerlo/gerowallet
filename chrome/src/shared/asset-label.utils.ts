const TABLE: Array<number> | Int32Array = [
  
];

function crc8(
  current: Uint8Array,
  previous = 0
): number {
  let crc = ~~previous;

  for (let index = 0; index < current.length; index++) {
    crc = TABLE[(crc ^ current[index]) & 0xff] & 0xff;
  }

  return crc;
}

function decodeString(str: string): Uint8Array {
  if (str.length % 2 !== 0) {
    throw new Error('Invalid hexadecimal string');
  }

  const result = new Uint8Array(str.length / 2);
  for (let i = 0; i < str.length; i += 2) {
    result[i / 2] = parseInt(str.substr(i, 2), 16);
  }

  return result;
}

function checksum(num: string): string {
  return crc8(decodeString(num)).toString(16).padStart(2, '0');
}

/*
* CIP-67 Implementation
* https://cips.cardano.org/cips/cip67/
* */
function extractLabel(assetName: string): string {
  return assetName.slice(0, 8);
}

function extractName(assetName: string): string {
  return assetName.slice(8, assetName.length);
}

function toLabel(num: number): string {
  if (num <= 0 || num > 65535) {
    throw new Error(
      `Label ${num} out of range: min label 1 - max label 65535.`
    );
  }
  const numHex = num.toString(16).padStart(4, '0');
  return '0' + numHex + checksum(numHex) + '0';
}

function fromLabel(label: string): number | undefined {
  if (label.length !== 8 || !(label[0] === '0' && label[7] === '0')) {
    return undefined;
  }

  const numHex = label.slice(1, 5);
  const num = parseInt(numHex, 16);
  const check = label.slice(5, 7);

  return check === checksum(numHex) ? num : undefined;
}

export const AssetLabelUtils = {
  extractLabel,
  extractName,
  toLabel,
  fromLabel
};
