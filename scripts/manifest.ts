...
async function getManifest() {
  ...
  const manifest: ManifestWithOAuth2 = {
    ...
    web_accessible_resources: [
      {
        resources: ["public/logo.png", "public/logo128.png", "content/_virtual_inject.js", "public/2.5.4.png"],
        matches: ["<all_urls>"]
      }
    ],
    content_scripts: [
    ...
    content_security_policy: {
      extension_pages: isDev ?
        `default-src 'self'; script-src 'self' 'wasm-unsafe-eval' http://localhost:*; font-src 'self' https://fonts.gstatic.com/ http://localhost:*; connect-src https://dev.gerowallet.io https://api.coingecko.com https://analytics-snekfun.splash.trade wss://*.ably-realtime.com https://*.ably-realtime.com https://*.ably.io wss://*.ably.io https://www.googleapis.com/oauth2/v3/userinfo https://api.handle.me/ https://media.bringweb3.io/ https://sandbox-api.bringweb3.io http://localhost:* ws://localhost:* https://fastly.jsdelivr.net/npm/@sec-ant/zxing-wasm@2.1.5/dist/reader/zxing_reader.wasm https://api.cardanoshield.com/api/ data:; style-src * 'unsafe-inline' 'self'  blob: ; img-src 'self'  http: data: ; frame-src http://localhost:* https://*.moonpay.com https://connect.trezor.io/; media-src http://localhost:* data:; object-src 'self'`
